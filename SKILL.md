...
- **用户提出“加速、赶时间、时间压力、临近截止、先省略、先给结果”等要求，或当前任务客观存在明显时间压力时：必须读取 `00_总览/05_赛时加速与流程保真协议.md`。时间压力只能改变调度、并行度、执行资源、非必要范围或 Claim 强度，不得降低本来必需的数据横向比较、参数证据、验证、Mathematical Review、Evidence Gate 或 Freeze 前置条件。**
- **进入新 Question、新模型路线冻结、正式结果验收或 Paper Handoff 前：读取 `00_总览/06_正赛技术红线速查.md` 做防漏扫描。该文件只负责快速发现风险，命中后必须回到对应 canonical Gate / workflow，不能用速查表替代正式审查。**
