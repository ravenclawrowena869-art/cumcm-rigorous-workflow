...
class RuntimeSkillContractTests(unittest.TestCase):
    ...
    def test_shared_core_has_math_authority_freeze_and_parameter_rule(self):
    ...
    def test_parameter_protocol_is_runtime_contract(self):
        text = (ROOT / "03_建模与代码" / "05_参数选择协议.md").read_text(encoding="utf-8")
        for token in (
            "先判断参数的数学身份",
            "参数依据的优先级",
            "文献优先寻找，但不是机械硬要求",
    ...
    def test_evidence_gate_requires_dynamic_replay_parameter_evidence_and_is_not_mechanical(self):
