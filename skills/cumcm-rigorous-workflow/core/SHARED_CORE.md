...
## CORE-PARAM-001 参数必须有证据链

任何 materially 影响可行性、目标、收敛、服务质量或论文核心 Claim 的经验参数，不得只以“经验取值”进入 Freeze。必须先说明参数的现实/数学/算法含义，并判断它是否真的需要固定；再优先依据题面、理论、**定义和尺度可比的**文献/标准、数据或 pilot 确定合理搜索范围；随后通过改参数重求解、自适应方案比较或其他与模型结构匹配的验证方法确定正式取值，并做邻域/跨场景复核。

文献只用于形成可信先验和缩小搜索范围，不能把不同定义参数的数值直接移植为本题结论；若没有直接可比文献，不得强行找相似数字凑引用，应改用理论边界 + 数据 pilot + 敏感性证据。粗粒度扫描只用于 screening，不等于最优性证明。步长、更新半径、每轮调整比例、阻尼和平滑系数等算法控制量必须至少讨论固定值与自适应规则哪一种更符合问题结构。

详细协议：`03_建模与代码/05_参数选择协议.md`。
