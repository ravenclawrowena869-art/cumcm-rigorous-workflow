...
4. **选择验证**：再去 Evidence Gate、参数选择协议或专属 validator 中选择合适的复算、扰动、对照、参数证据或边界实验。
...
本文件负责“发现并归类风险”；Evidence Gate、参数选择协议和专属 validator 负责“如何验证”。若出现现有十类无法自然容纳、但可能实质影响正式结果的新风险，记录为 `CUSTOM_RISK`，不要为了套模板硬塞进 CG1–CG10，也不要立刻扩充成新的长期 Gate；先在当前任务中验证其重要性，再决定是否值得沉淀为通用规则。
